...
export class ProductForm extends React.Component
{
    ...
    {   
        ...
        if(this.validate())
        {
            this.props.getTableContents({name:this.refs.nameInput.value,price:this.refs.priceInput.value,quantity:this.refs.quantityInput.value})            
        }
    }
    ...
    render()
    {
        return (
            <div>
                <form>
                    <strong>&nbsp;Name :</strong> <input ref="nameInput"type="text" placeholder="Enter Name" onChange={this.OnNameInputChange.bind(this)}/>
                    <p style={{color:'red',marginLeft:'20px'}}>{this.state.errorName}</p>
                    <br/>
                    <strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Quantity :  </strong> <input ref="quantityInput"type="text" placeholder="Enter Quantity" onChange={this.OnQuantityInputChange.bind(this)}/>
                    <p style={{color:'red',marginLeft:'20px'}}>{this.state.errorQuantity}</p>
                    <br/>
                    <strong>Price:  </strong> <input ref="priceInput"type="text" placeholder="Enter Price" onChange={this.OnPriceInputChange.bind(this)}/>
                    <p style={{color:'red',marginLeft:'20px'}}>{this.state.errorPrice}</p>
                    <br/>
                    <input style={{marginLeft:'50px'}} type="submit" ref="add-product-button" onClick={this.onAddProducrButtonClicked.bind(this)} value="Add Product"/>
                </form>
                <hr/>
            </div>
